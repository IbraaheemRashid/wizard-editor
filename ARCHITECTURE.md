...
A native macOS video editor built in **Rust + egui + wgpu**, targeting Apple Silicon with a Metal backend. Three-panel architecture (media browser + timeline + preview) with real-time video playback, audio mixing, GPU-accelerated waveforms, and 60fps rendering — all from a single cross-platform codebase.

**~14,400 lines of Rust** across 5 crates. No Electron. No native OS widgets. Pure GPU-rendered UI.
...
graph LR
    ...
    media["wizard-media<br/><i>GStreamer, decode</i>"]
...
| ---------------------- | ------------------------------------------- | ------ | ---------------------------------------------------------------------------- |
| **wizard-app**   | Binary entry point, run loop, orchestration | ~3,900 | eframe, egui, notify, ringbuf                                                |
| **wizard-state** | Shared data models, no UI or media deps     | ~1,400 | uuid (only)                                                                  |
| **wizard-ui**    | All egui panel rendering                    | ~2,800 | egui, egui-wgpu, wgpu, rfd                                                   |
| **wizard-media** | GStreamer decoding, thumbnails, pipelines   | ~1,800 | gstreamer, gstreamer-app, gstreamer-video, gstreamer-pbutils, image, ringbuf |
| **wizard-audio** | cpal audio device output                    | ~150   | cpal, ringbuf                                                                |
...
| Display thumbnails in a grid   | Done   | Async extraction via GStreamer on background threads, GPU texture upload, responsive grid layout that adapts to panel width                                                                         |
...
| Playing timeline shows frame in preview       | Done   | Full decode pipeline: playhead position -> timeline clip lookup -> source time mapping -> GStreamer decode -> GPU texture -> preview panel |
...
flowchart TD
    ...
    C --> D["spawn thread"]
    D --> E["extract_metadata()<br/><i>GStreamer Discoverer</i>"]
    D --> F["extract_thumbnail()<br/><i>GStreamer pipeline</i>"]
...
User drags folder ──► rfd file dialog
     ...
     (gst       (gst        (decode all audio
      Discoverer) pipeline)   via gst)
         │         │         │
         ▼         ▼         ▼
...
│    1s mono snippet decode    GStreamer Pipeline (per clip):          │
│    fade in/out               filesrc → decodebin → appsinks         │
│    → audio.snippet_rx          video bridge → frame channel         │
│                                audio bridge → ringbuf               │
│                                                                      │
│  cpal Audio Thread:                                                  │
...
flowchart TD
    ...
    subgraph bg["Background Threads"]
        ...
        W6["GStreamer pipeline<br/><i>internal threads</i>"]
        W7["Video bridge<br/><i>appsink → channel</i>"]
        W8["Audio bridge<br/><i>appsink → ringbuf</i>"]
    end
...
```mermaid
flowchart TD
    Start["GstPipelineHandle::start()"] --> GST["GStreamer Pipeline<br/><i>filesrc → decodebin</i>"]
    GST --> VBridge["Video bridge thread<br/><i>appsink → channel</i>"]
    GST --> ABridge["Audio bridge thread<br/><i>appsink → ringbuf</i>"]

    VBridge --> FrameRx["frame_rx<br/><i>mpsc channel</i>"]
    ABridge --> RingBuf["ringbuf<br/><i>lock-free producer</i>"]
...
```
             ...
             + videoscale    + audioresample
             + appsink       + appsink
                    │             │
             Video bridge    Audio bridge
             thread          thread
                    │             │
                    ▼             ▼
              frame_rx       ringbuf
...
GStreamer manages demuxing and decoding internally via its pipeline graph (`filesrc → decodebin → {videoconvert + videoscale + appsink, audioconvert + audioresample + appsink}`). Bridge threads pull from GStreamer appsinks and forward to the main thread via channels (video) or ring buffers (audio). The main thread never touches GStreamer directly — it only polls channels.

**PTS-based pacing:** GStreamer's pipeline handles frame timing internally when in `Playing` state. For scrubbing and variable-speed playback, we use `seek` events with rate parameters. Speed changes rebase without dropping frames.
...
# Prerequisites
# - Rust (via rustup)
# - GStreamer (brew install gstreamer gst-plugins-base gst-plugins-good gst-plugins-bad gst-plugins-ugly gst-libav)
...
GStreamer is used for all media operations (video/audio decoding, metadata extraction, thumbnail generation, waveform analysis). The `gst-libav` plugin provides FFmpeg-backed codecs through GStreamer's plugin system, giving broad codec support without direct FFmpeg bindings.
...
| **Video Decode**  | GStreamer 0.24 (gstreamer-rs) | Pipeline-based media framework — demux, decode, scale, resample via declarative graphs. Codec support via gst-libav plugin (FFmpeg codecs through GStreamer's plugin system) |
...
### Why GStreamer, Not Direct FFmpeg Bindings

The initial implementation used `ffmpeg-the-third` (direct Rust bindings to libavformat/libavcodec). We migrated to GStreamer for several reasons:

1. **Pipeline-as-graph model** — GStreamer's `filesrc → decodebin → {videoconvert + appsink, audioconvert + appsink}` declarative graph replaces manual demuxer/decoder thread management. The framework handles demuxing, codec selection, and format negotiation automatically.
2. **Seeking is a first-class primitive** — GStreamer's `seek` event with rate and flags handles variable-speed playback, reverse playback, and frame-accurate seeking through a single API. With direct FFmpeg bindings, we had to manage keyframe indexes, PTS tracking, and scaler context caching manually.
3. **Plugin ecosystem** — GStreamer's `gst-libav` plugin provides FFmpeg-backed codecs without us managing the FFmpeg lifecycle. Hardware-accelerated decoders (VideoToolbox on macOS) are available as drop-in plugins.
4. **Mature Rust bindings** — `gstreamer-rs` 0.24 provides safe, idiomatic Rust wrappers. The crate is actively maintained and well-documented.

**What we gave up:** Direct frame-level control. With FFmpeg bindings, we could call `decoder.decode(packet)` and get a frame. With GStreamer, we describe a pipeline and pull from appsinks. For reverse playback, this means we build a short-lived pipeline per GOP window, decode all frames, reverse them, and pace them ourselves — more orchestration code, but the individual decode step is simpler.

**What we kept:** The threading model is identical. GStreamer pipelines run on their own threads; bridge threads pull from appsinks and push to channels/ring buffers; the main thread polls with `try_recv()`. The 60fps guarantee is unaffected because the main thread never touches GStreamer directly.

See [DECISIONS.md](DECISIONS.md) for the full tradeoff analysis.
